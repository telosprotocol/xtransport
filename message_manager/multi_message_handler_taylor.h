...
{
public:
    explicit ThreadHandler(base::xiothread_t * raw_thread_ptr);
...
class MultiThreadHandler : public std::enable_shared_from_this<MultiThreadHandler>
{
    enum
    {
        enum_const_woker_threads_count = 4,//usally 1 worker thread to handle packet
    };
public:
    MultiThreadHandler();
    ~MultiThreadHandler();
...
private:
    ThreadHandler*  m_worker_threads[enum_const_woker_threads_count];
};
